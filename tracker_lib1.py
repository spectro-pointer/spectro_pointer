...
SIZE = (316,262)   #(308, 254)  # Camera resolution in (x, y)
X_OFFSET = 0
Y_OFFSET = 0
...
def set_up_camera():
    ...
    try:
      #  camera = PiCamera(resolution=SIZE, framerate=Fraction(fps, 1))
...
def create_coordinates(image):
    ...
    cv2.line(image, (0, SIZE[1]/2), (SIZE[0], SIZE[1]/2 + Y_OFFSET), (255, 0, 0), 1)
    cv2.line(image, (SIZE[0]/2, 0), (SIZE[0]/2, SIZE[1]+ X_OFFSET), (0,255, 0), 1)# vertical
    ...
    cv2.circle(image, (155,125),5 , (0, 255, 0),1 ) 
    return image
...
def check_quadrant(cx, cy):
    ...
    if abs(cx - SIZE[0]/2) < CENTER_RADIUS:
        ...
        result = "left"
        print "left"
        azimuth_controller.move_left(1) #codigo insertado
        led_action(available_leds["LED_G_LEFT"], "on")
        led_action(available_leds["LED_G_RIGHT"], "off")
    elif cx > SIZE[0]/2:
        result = "right"
        print "right"
        azimuth_controller.move_right(1) # codigo insertado
        led_action(available_leds["LED_G_LEFT"], "off")
